...
static int skipcomment (LoadF *lf, int *cp) {
  ...
  if (c == '#') {  /* first line is a comment (Unix exec. file)? */
    do {  /* skip first line */
    	f_gets((char*)&c,1,&lf->f);
    } while (c != EOF && c != '\n');
    f_gets((char*)cp,1,&lf->f);  /* skip end-of-line, if present */
  ...
  if (filename == NULL) {
   ...
   // lf.f = stdin;
    ...
    lua_pushfstring(L, "@%s", filename);
     f_open(&lf.f,filename, FA_READ);
    if (lf.f.fs == NULL) return errfile(L, "open", fnameindex);
  }
  if (skipcomment(&lf, &c))  /* read initial portion */
    lf.buff[lf.n++] = '\n';  /* add line to correct line numbers */
  if (c == LUA_SIGNATURE[0] && filename) {  /* binary file? */
    f_reopen(filename, FA_READ, &lf.f);  /* reopen in binary mode */
    if (lf.f.fs == NULL) return errfile(L, "reopen", fnameindex);
